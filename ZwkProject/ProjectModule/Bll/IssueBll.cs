...
namespace ProjectModule.Bll
{
    public class IssueBll
    {
        ...
        public async Task<PageData<Issue>> GetListAsync(string projectId, string scheduleId, string principalId, string orderby, int page, int size)
        {
            Expression<Func<Issue, bool>> where = p => p.ProjectId == projectId;
            if (!string.IsNullOrEmpty(scheduleId))
                where = where.And(p => p.ScheduleId == scheduleId);
            if (!string.IsNullOrEmpty(principalId))
                where = where.And(p => p.PrincipalId == principalId);
